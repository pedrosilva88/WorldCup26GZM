...
// forked from design-sync lib/bundle.mjs — stub Next.js modules (next/link, next/navigation) for WorldCupGZM app components

// Shim for Next.js modules not available in the design-sync runtime
const nextShim = {
  name: 'next-shim',
  setup(b) {
    ...
    b.onLoad({ filter: /^next-link-shim$/, namespace: 'shim' }, () => ({
      contents: `var R=window.React;
module.exports=function Link(p){return R.createElement('a',Object.assign({},p,{href:p.href||'#'}),p.children);};`,
      loader: 'js',
    }));
    b.onLoad({ filter: /^next-nav-shim$/, namespace: 'shim' }, () => ({
      contents: `exports.useRouter=function(){return{push:function(){},replace:function(){},back:function(){},refresh:function(){},forward:function(){}};};
...
exports.useParams=function(){return {};};`,
    ...
    b.onLoad({ filter: /^next-image-shim$/, namespace: 'shim' }, () => ({
      contents: `var R=window.React;
module.exports=function Image(p){return R.createElement('img',Object.assign({},p,{src:p.src||''}));};`,
      loader: 'js',
    }));
  },
};
