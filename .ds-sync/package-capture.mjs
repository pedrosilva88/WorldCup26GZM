...
// authored .tsx or cfg.previewArgs). Floor-card components are the
// validator's territory (.render-check.json `fallbackCard`), not graded.
//
// LIFECYCLE — one invariant: a grade lives until its contract changes.
// gradeKey = styleSha + renderHash (lib/sync-hashes.mjs — the same recipe
// the uploaded _ds_sync.json sidecar carries). Unchanged & fully graded
// `good` → carried forward within the campaign, zero work.
//
...
{
  const VALUE_FLAGS = ['out', 'components'];
...
// The package shape includes the DS bundle body in styleSha: with no
// reference render, the absolute judgment rests on the compiled code too.
const STYLE_SHA = styleShaFor(OUT, { includeBundleBody: true });
const gradeKeyFor = (c) => gradeKeyFrom(STYLE_SHA, renderHashFor(OUT, c, {}));
...
const comps = ONLY ? all.filter((c) => ONLY.has(c.name)) : all;
...
for (const c of comps) {
  ...
  if (cells.length === 0) {
    ...
    console.error(`✗ [CAPTURE] ${c.name}: ${why} — fix the preview (.design-sync/previews/${c.name}.tsx if you authored it; otherwise cfg.previewArgs/stories regenerate .design-sync/.cache/previews/${c.name}.tsx) and rebuild`);
    continue;
  }

  const fullyGraded = grade?.cells && cells.length > 0
    && cells.every((k) => ['good'].includes(grade.cells[k]?.verdict));
  if (!FORCE && fullyGraded && prev?.gradeKey === gradeKey) {
  ...
  writeFileSync(capPath, JSON.stringify({ gradeKey, cells, shots: shots.map((s) => s.label), pageErrs: [...new Set(pageErrs)].slice(0, 3) }, null, 2));
