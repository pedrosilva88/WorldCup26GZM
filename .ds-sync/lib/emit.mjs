...
import { escapeHtml, readText } from './common.mjs';
...
<html><head><meta charset="utf-8">
  ...
  <script>
    ...
    var MODE=${JSON.stringify(card.cardMode === 'single' ? 'single' : 'grid')};
  ...
  const providerProps = (props, G) => {
    const pairs = Object.entries(props ?? {}).map(([k, v]) => {
      ...
      if (v && typeof v.$ref === 'string') {
        if (/^[A-Za-z_$][\w$.]*$/.test(v.$ref)) return `${JSON.stringify(k)}:${G}.${v.$ref}`;
...
export function emitPerComponent({ src, components, OUT, GLOBAL, PKG, VERSION, OVERRIDES, REPLACES, PROVIDER, hasDecorators, exported, builtPreviews, propsBodyFor, compoundsFor, smartDefaultProps, previewArgs }) {
  // Allow dotted paths (`ThemeContext.Provider`) — exported-check is on the head.
  ...
  for (const c of components) {
    ...
    // cfg.previewArgs > .d.ts smart-defaults.
    ...
    // Single-mode cards declare a viewport so the product renders the card at
    // the same size the compare harness grades at (default = the harness
    // capture viewport).
...
export function emitReadme({ OUT, GLOBAL, PKG, VERSION, TOKENS_PKG, components, tokenFiles, hasProvider, PROVIDER, jsdocFor, compoundsFor, guidelineCount = 0 }) {
