...
export function writePreviewFiles({ components, previewDir, genDir, gen }) {
  ...
  for (const c of components) {
    ...
    if (ownedTxt !== null) {
      ...
      // comment, the file compiles fine): warn, don't act.
      const nl = ownedTxt.indexOf('\n');
      if (MARKER_RE.test(nl < 0 ? ownedTxt : ownedTxt.slice(0, nl))) {
        console.error(`  (note: ${c.name}.tsx in .design-sync/previews/ still carries the generated marker on line 1 — delete the line; owned files are markerless)`);
      }
    ...
    if (genTxt !== null) {
      ...
      if (!m || (m[1] && m[1] !== bodyHash(nl < 0 ? '' : genTxt.slice(nl + 1)))) {
        modified++;
        console.error(`  (preview modified in the cache: ${c.name} — NOT regenerating over it; it is gitignored here and vanishes on a fresh clone — move it to .design-sync/previews/${c.name}.tsx, minus any marker line, to own it durably)`);
        continue;
      }
    }
    const body = gen(c);
