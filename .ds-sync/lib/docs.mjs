...
export function discoverDocs({ components, PKG_DIR, cfg, cfgPath }) {
  ...
  for (const c of components) {
    const p = findComponentDoc(c, { docsDirFiles, mapped: cfg.docsMap?.[c.name], cfgPath });
    if (p && existsSync(p)) { c.docPath = p; matched++; }
    else missed.push(c.name);
  }
  console.error(`  docs: ${matched}/${components.length} components matched${docsDir ? ` (cfg.docsDir=${relative(PKG_DIR, docsDir) || '.'})` : ''}`);
