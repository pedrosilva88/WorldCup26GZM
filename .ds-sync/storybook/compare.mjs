...
//
// LIFECYCLE — one invariant: a grade is valid while its CONTRACT is
// unchanged. The grade key = story file (srcSha) + preview source
// (compiled js/html) + preview styling (css/fonts/tokens/decorators).
// - Grade key unchanged + fully graded match/close → skipped outright
//   ("carried forward"); no capture. Bundle/storybook rebuilds alone never
//   invalidate — both sides render the same compiled code, in lockstep.
...
//   semantics, and is honored on scoped runs too (the §7.3 re-sync flow).
...
import { escapeHtml, exportName } from '../lib/common.mjs';
import { gradeKeyFrom, renderHashFor, styleShaFor } from '../lib/sync-hashes.mjs';
...
// Keys come from the shared recipe (lib/sync-hashes.mjs) — the SAME hashes
// package-build writes into the uploaded _ds_sync.json sidecar, so local
// grade carry-forward and remote verified-by-upload can never disagree.
// The storybook shape EXCLUDES the bundle JS from styleSha: both panels
// render the same compiled code, so they move in lockstep and the fidelity
// judgment is unaffected. Pixel comparisons are not used at all.
const STYLE_SHA = styleShaFor(OUT, { includeBundleBody: false });
const gradeKeyFor = (c) => gradeKeyFrom(STYLE_SHA, renderHashFor(OUT, c, { stories: c.stories, srcSha: c.srcSha ?? null }));
// Recorded only to power the [REFERENCE_STALE?] hint (bundle moved without a
// reference rebuild ⇒ grading would chase the old design). Not a skip input.
// project.json carries storybook's generatedAt timestamp — excluded.
const sbH = createHash('sha256');
hashDir(sbH, SB, 'sb/', new Set(['project.json']));
const SB_BASE_SHA = sbH.digest('hex');
...
// sampler is off (§7.3 re-syncs name picks from the diff's `unchanged` set).
...
if (spotChecks.size) {
  console.error(`◉ [SPOT_CHECK] re-verifying ${spotChecks.size} requested carried-forward component(s): ${[...spotChecks].join(', ')} — grades kept; Read their fresh sheets and confirm they still match the recorded grades. A divergence here is SYSTEMIC (build skew / converter regression), not a component bug: stop, diagnose, then --force a full pass.`);
}
if (!ONLY && !FORCE && SPOT_CHECK_N > 0) {
  ...
  if (sampled.length) {
    console.error(`◉ [SPOT_CHECK] re-verifying ${sampled.length} carried-forward component(s) after shared-input changes: ${sampled.join(', ')} — Read their fresh sheets and confirm they still match the recorded grades. A divergence here is SYSTEMIC (build skew / converter regression), not a component bug: stop, diagnose, then --force a full pass.`);
  }
}
...
try {
  ...
  for (const c of comps) {
    ...
    const { gradeKey, prevCapture, grade, fullyGraded } = pre.get(c.name);
    ...
    // same styling. Bundle/storybook rebuilds alone don't invalidate (both
    // sides render the same compiled code — lockstep). No capture at all;
    ...
    if (!FORCE && fullyGraded && prevCapture?.gradeKey === gradeKey && !spotChecks.has(c.name)) {
      ...
      console.error(`↻ [COMPARE] ${c.name}: story contract, preview, and styling unchanged & fully graded — carried forward (--force to recapture)`);
      continue;
    }
    ...
    // Clear the old grade only when the contract it judged actually changed
    // (or on --force, where the point is a fresh verdict). A PARTIAL grade on
    // an unchanged contract stays — those verdicts are still valid; the
    // component is only being recaptured because it isn't fully graded yet.
    ...
    // (Preview from cfg.previewArgs, a Variants grid, a fallthrough Default)
    ...
    const entry = {
      name: c.name, group: c.group, previewKind: pv.kind, portal: pv.portal,
