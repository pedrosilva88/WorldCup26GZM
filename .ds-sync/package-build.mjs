...
for (const ep of extraEntries) {
  try {
    ...
    const collisions = [...exportedNames(dir, pj)].filter((n) => exported.has(n));
...
// A provider component that isn't on the global fails EVERY preview
// identically ("Element type is invalid: undefined") — catch it at build.
// Dotted paths check their head; DSes often prefix experimental exports
// (unstable_*), so the plain name may simply not exist.
for (let p = PROVIDER; p; p = p.inner) {
  const head = String(p.component ?? '').split('.')[0];
  if (head && !exported.has(head)) {
    console.error(`! [PROVIDER_UNEXPORTED] cfg.provider component "${p.component}" is not a bundle export — every preview will fail with "Element type is invalid". Check the exact exported name (prefixed variants like unstable_${head} are common).`);
  }
}
...
writePreviewFiles({
  ...
  gen: (c) => generatePreviewSource(c, {
    // smartDefaultProps is a full ts-morph props resolution — only the
    // package-shape generator reads it.
    smart: shape === 'storybook' ? undefined : smartDefaultProps(c.name, propsBodyFor(c.name, { ...dts, dtsPropsFor: cfg.dtsPropsFor })),
    exported, pkg: PKG,
...
writeFileSync(
  ...
  JSON.stringify({
    ...
    pkgDir: PKG_DIR,
    extraEntries,
    // For preview-rebuild's provider gate — must match emitPerComponent's
    // `exported.has(...)` check so a targeted rebuild emits the same html.
...
// sidecar, so re-syncs never download the full bundle). styleSha/renderHashes
// use the SAME recipe the grading harnesses key on (lib/sync-hashes.mjs):
// a component whose renderHash matches the uploaded sidecar — under an
// unchanged styleSha — renders byte-identically to what was verified at the
// last upload, so it needs no re-verification. Uploaded in the same fenced
// plan as the bundle; off-script layout generators must produce it too.
// Written LAST so every hashed surface (README included — auxSha) exists.
{
  ...
  writeFileSync(join(OUT, '_ds_sync.json'), JSON.stringify({ shape, styleSha, renderHashes, sourceHashes: headerMeta.sourceHashes, auxSha: auxShaFor(OUT), bundleSha12 }, null, 2) + '\n');
  console.error(`  _ds_sync.json: ${components.length} render hash(es) (verification anchor)`);
}
