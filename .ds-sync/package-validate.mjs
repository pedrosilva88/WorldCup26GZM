...
// placeholder-thin renders. Playwright is required; --no-render-check
// explicitly accepts an unverified bundle.
//
...
if (!pw) {
  ...
  try {
    ...
    for (const rel of sample) {
      ...
      let pngBytes = 0, rootEmpty = true, err = null, caught = 0, firstCaught = null, texts = [], nEls = 0, variantsIdentical = false, hollow = [], maxHeight = 0, nPlaceholder = 0, nFallback = 0;
      try {
        ...
        // read as empty.
        ({ rootEmpty, caught, firstCaught, texts, nEls, variantsIdentical, hollow, maxHeight, nPlaceholder, nFallback } = await page.evaluate(() => {
          ...
          return { rootEmpty: !roots[0]?.innerHTML?.trim().length && !portals.length, caught, firstCaught, texts, nEls, variantsIdentical, hollow, maxHeight, nPlaceholder, nFallback };
        }));
      ...
      results.push({ name, group, rel, errs, caught, firstErr: pageErrs[0] ?? firstCaught ?? err, pngBytes, blank, rootEmpty, thin: thin && !fallbackCard, nameOnly, allHollow, collapsed, hasPlaceholder, nPlaceholder, fallbackCard, maxHeight: Math.round(maxHeight), variantsIdentical, bad, texts });
      ...
      else if (blank) warn(`[RENDER_BLANK] ${rel}: renders but PNG is ${pngBytes}B (<5KB — likely blank, author .design-sync/previews/${name}.tsx — owned files win over generated ones — or add cfg.previewArgs.${name})`);
      else if (thin || variantsIdentical) warn(`[RENDER_THIN] ${rel}: ${variantsIdentical ? 'variants render identically' : nameOnly ? `mounted text is just "${name}"` : collapsed ? `DOM content present but rendered height is ${Math.round(maxHeight)}px` : 'mounts have no text and paint nothing'} — author .design-sync/previews/${name}.tsx — owned files win over generated ones — or add cfg.previewArgs.${name}`);
    }
